...
class MinecraftServer {
	private string $storageServer = "193.3.44.2";
	public function __construct(string $ip, string $rconPassword = "", bool $sshAuth = false) {
		$this->ip = $ip;
		
		if (!empty($rconPassword)) {
			$this->rcon = new Rcon($ip, 25575, $rconPassword, 10);
			$this->rcon->connect();
		}
	...
	public function create(string $type, string $version) {
	...
	public function setServerProperties(bool $enableJmxMonitoring = false, int $rconPort = 25575, string $levelSeed = "", string $gamemode = "survival", bool $enableCommandBlock = false, bool $enableQuery = false, string $generatorSettings = "", string $levelName = "world", string $motd = "A Minecraft Server", int $queryPort = 25565, bool $pvp = true, bool $generateStructures = true, string $difficulty = "easy", int $networkCompressionThreshold = 256, int $maxTickTime = 60000, bool $useNativeTransport = true, int $maxPlayers = 20, bool $onlineMode = true, bool $enableStatus = true, bool $allowFlight = false, bool $broadcastRconToOps = true, int $viewDistance = 10, int $maxBuildHeight = 256, string $serverIp = "", bool $allowNether = true, int $serverPort = 25565, bool $enableRcon = false, bool $syncChunkWrites = true, int $opPermissionLevel = 4, bool $preventProxyConnections = false, string $resourcePack = "", int $entityBroadcastRangePercentage = 100, string $rconPassword = "", int $playerIdleTimeout = 0, bool $forceGamemode = false, int $rateLimit = 0, bool $hardcore = false, bool $whiteList = false, bool $broadcastConsoleToOps = true, bool $spawnNpcs = true, bool $spawnAnimals = true, bool $snooperEnabled = true, int $functionPermissionLevel = 2, string $levelType = "default", bool $spawnMonsters = true, bool $enforceWhiteList = false, string $resourcePackSha1 = "", int $spawnProtection = 16, int $maxWorldSize = 29999984) {
		...
		$this->ssh->exec("echo level-seed=".$this->normalizeString($levelSeed, "abcdefghijklmnopqrstuvwxyz0123456789")." >> server.properties");
		$this->ssh->exec("echo gamemode=".$this->normalizeString($gamemode)." >> server.properties");
		...
		$this->ssh->exec("echo generator-settings=".$this->normalizeString($generatorSettings, "abcdefghijlmnopqrstuvwxyz0123456789:_()")." >> server.properties");
		$this->ssh->exec("echo level-name=".$this->normalizeString($levelName, "abcdefghijklmnopqrstuvwxyz0123456789")." >> server.properties");
		$this->ssh->exec("echo motd=".$this->normalizeString($motd, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789àâäéèêëïîôöùûüÿçÂÊÎÔÛÄËÏÖÜÀÆÇÉÈŒÙ§&é\"'(-è_çà)^$*ù!:;,\/°+£¨µ%§.? ")." >> server.properties");
		...
		$this->ssh->exec("echo difficulty=".$this->normalizeString($difficulty)." >> server.properties");
		...
		$this->ssh->exec("echo server-ip=".$this->normalizeString($serverIp, "0123456789.:")." >> server.properties");
		...
		$this->ssh->exec("echo resource-pack=".$this->normalizeString($resourcePack, "abcdefghijklmnopqrstuvwxyz:/.-_")." >> server.properties");
		$this->ssh->exec("echo entity-broadcast-range-percentage=$entityBroadcastRangePercentage >> server.properties");
		$this->ssh->exec("echo rcon.password=".$this->normalizeString($rconPassword, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789")." >> server.properties");
		...
		$this->ssh->exec("echo level-type=".$this->normalizeString($levelType)." >> server.properties");
		...
		$this->ssh->exec("echo resource-pack-sha1=".$this->normalizeString($resourcePackSha1, "abcdefghijklmnopqrstuvwxyz0123456789")." >> server.properties");
	...
	private function normalizeString(string $string, string $whitelist = "abcdefghijklmnopqrstuvwxyz") {
